...
public class MobileleUserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        UserEntity userFromDb = userRepository
                .findByUsername(username)
                .orElseThrow(()-> new UsernameNotFoundException("User "+ username + " not found!"));

        List<GrantedAuthority> authorities = userFromDb
                .getUserRoles()
                .stream()
                .map(r-> new SimpleGrantedAuthority("ROLE_" + r.getRole().name()))
                .collect(Collectors.toList());
        return mapToUserDetails(userFromDb,authorities);
    }
    private UserDetails mapToUserDetails(UserEntity entity, List<GrantedAuthority> authorities){
